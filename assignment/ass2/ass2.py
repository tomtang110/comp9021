...
class Frieze:
    ...
    def _glided(self):
        ...
        return True
    def _rotation(self):
        self.r_heng = deepcopy(self.single_period_heng)
        self.r_shu  = deepcopy(self.single_period_shu)
        self.r_SE = deepcopy(self.single_period_SE)
        self.r_NE = deepcopy(self.single_period_NE)
        ...
        self.r_NE.reverse()
        for h in range(self.height):
            self.r_heng[h].reverse()
            self.r_shu[h].reverse()
            self.r_SE[h].reverse()
            self.r_NE[h].reverse()
        estimate = True
        # heng
        if self.r_heng != self.single_period_heng:
            estimate =False
        # shu 
        for h in range(self.height-1):
            for l in range(self.period_num-1):
                if self.r_shu[h][l] != self.single_period_shu[h+1][l+1]:
                    estimate = False
        # SE
        for h in range(self.height-1):
            if self.r_SE[h+1] != self.single_period_SE[h]:
                estimate = False
        # NE 
        for h in range(self.height-1):
            if self.r_NE[h] != self.single_period_NE[h+1]:
                estimate = False
        
        if estimate:
            return True
        else:
            finalr = self._rotation2()
            return finalr
    def _rotation2(self):
        estimate = False
        self.ro_heng_r = deepcopy(self.single_period_heng)
        self.ro_shu_r = deepcopy(self.single_period_shu)
        self.ro_NE_r = deepcopy(self.single_period_NE)
        self.ro_SE_r = deepcopy(self.single_period_SE)
        for g in range(self.period_num//2+1):
            if not estimate:
                for h in range(self.height):
                    period_heng1 = self.ro_heng_r[h].pop()
                    self.ro_heng_r[h].insert(0,period_heng1)
                self.ro_heng_rr = deepcopy(self.ro_heng_r)
                self.ro_heng_rr.reverse()
                for k in self.ro_heng_rr:
                    k.reverse()
                for h in range(self.height):
                    period_shu1 = self.ro_shu_r[h].pop()
                    self.ro_shu_r[h].insert(0,period_shu1)
                self.ro_shu_rr = deepcopy(self.ro_shu_r)
                self.ro_shu_rr.reverse()
                for k in self.ro_shu_rr:
                    k.reverse()
                for h in range(self.height):
                    period_NE1 = self.ro_NE_r[h].pop()
                    self.ro_NE_r[h].insert(0,period_NE1)
                self.ro_NE_rr = deepcopy(self.ro_NE_r)
                self.ro_NE_rr.reverse()
                for k in self.ro_NE_rr:
                    k.reverse()
                for h in range(self.height):
                    period_SE =self.ro_SE_r[h].pop()
                    self.ro_SE_r[h].insert(0,period_SE)
                self.ro_SE_rr = deepcopy(self.ro_SE_r)
                self.ro_SE_rr.reverse()
                for k in self.ro_SE_rr:
                    k.reverse()
                heng_count= 0
                heng_count1 = 0
                for h in range(self.height):
                    if heng_count == 0:
                        for l in range(self.period_num-1):
                            if self.ro_heng_r[h][l] != self.ro_heng_rr[h][l+1]:
                                estimate = False
                                heng_count += 1
                                break
                    else:
                ...
                shu_count = 0
                for h in range(self.height-1):
                    if self.ro_shu_r[h+1] != self.ro_shu_rr[h]:
                        estimate = False
                        shu_count += 1
                        break
                if shu_count > 0 :
                ...
                for h in range(self.height-1):
                    if SE_count == 0:
                            if self.ro_SE_r[h] != self.ro_SE_rr[h+1]:
                                estimate = False
                                SE_count += 1
                                break
                if SE_count != 0:
                ...
                for h in range(self.height-1):
                    if NE_count == 0:
                        if self.ro_NE_r[h+1] != self.ro_NE_rr[h]:
                                estimate = False
                                NE_count += 1
                                break
                if NE_count != 0:
