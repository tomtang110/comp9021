            ...
            if len(road1) == 1 or road1[-1][0] == road1[-2][0]:
                ...
                if 0 <=k < grid_len:       
                    if (k,j) not in road1:
                        branch2 = grid[k][j]
                        if branch2 == 1:
                            queue.enqueue((road1+[(k,j)],(k,j)))
                if 0 <= g < grid_len:
                    if (i,g) not in road1:
                        branch1 = grid[i][g]
                        if branch1 == 1:
                            queue.enqueue((road1+[(i,g)],(i,g)))
                if grid_len>h >= 0:
                    if (h,j) not in road1:
                        branch4 = grid[h][j]
                        if branch4 == 1:
                            queue.enqueue((road1+[(h,j)],(h,j)))
                ...
                if 0 <=k < grid_len:       
                    if (i,k) not in road1:
                        branch2 = grid[i][k]
                        if branch2 == 1:
                            queue.enqueue((road1+[(i,k)],(i,k)))
                if 0 <= g < grid_len:
                    if (g,j) not in road1:
                        branch1 = grid[g][j]
                        if branch1 == 1:
                            queue.enqueue((road1+[(g,j)],(g,j)))
                if grid_len>h >= 0:
                    if (i,h) not in road1:
                        branch4 = grid[i][h]
                        if branch4 == 1:
                            queue.enqueue((road1+[(i,h)],(i,h)))
        return road1
