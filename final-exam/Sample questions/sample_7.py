...
def find_word_vertically(grid, word):
    trans=[[None]*len(grid) for i in range(len(grid[0]))]
    for i in range(len(grid[0])):
        for j in range(len(grid)):
            trans[i][j] = grid[j][i]
    for i in range(len(trans)):
        for j in range(len(trans[i])-len(word)+1):
            if word == ''.join(trans[i][j:j+len(word)]):
                return j+1,i+1
